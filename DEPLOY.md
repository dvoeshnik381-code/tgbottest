...
Установщик запросит токен BotFather скрытым вводом, установит Node.js, затем создаст службу `tgbottest`. Повторный запуск команды обновляет код и сохраняет `.env` и каталог `data/`. Загрузка работает и на серверах, у которых доступен только IPv6.
